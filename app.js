...
ifHeCared.answer1 = function (question1Answer) {
    $('.cta1').on('click', function (event) {
        ...
        if (question1Answer === 'enivorment') {
            ...
            alert('Please select an option ... if you want you can go too, thanks for comming :)')
        }
    })
}
...
ifHeCared.displayResults = function (question3Answer) {
    $('.cta3').on('click', function (event) {
        ...
        if (question3Answer === 'ProtectingForests') {
            $('.land').addClass('hidden')
            $('.results').html('<h2>Save the Rainforest!</h2> <p>Jeff Bezos could save the amazon rainforest almost 3 times over find out more with his networth <a href="https://www.aljazeera.com/programmes/countingthecost/2019/09/amazon-rainforest-worth-190907121817405.html" target="_blank">more information here</a><p>')

        } else if (question3Answer === 'greenEnergy') {
            $('.land').addClass('hidden')
            $('.results').html('<h2>Build Green Energy</h2> <p>With Jeff Bezos\'s net worth he could build enough solar farms to power over 36 million homes <a href="https://energycentral.com/c/gn/initial-steps-building-solar-farm" target="_blank">more information here</a></p>')

        } else if (question3Answer === 'savingReef') {
            $('.water').addClass('hidden')
            $('.results').html('<h2>Save the Great Barrior Reef!</h2> <p>With Jeff Bezos\'s networth he could save the great berrior reef over 22 times <a href="https://www.abc.net.au/news/2016-08-11/great-barrier-reef-cost-to-save-reef-tops-8-billion-dollars/7721652" target="_blank">more information here</a></p>')

        } else if (question3Answer === 'trashIsland') {
            $('.water').addClass('hidden')
            $('.results').html('<h2>Clean-up pasific trash island<p>Jeff Bezos\'s networth (on saturday september 12th 2020) could hire enough boats to clean out the pasific trash island many times <a href="https://www.logisticsmgmt.com/article/moore_on_pricing_the_cost_of_ocean_pollution" target="_blank">more information here</a><a href="https://www.nbcsandiego.com/news/local/study-shows-the-great-pacific-garbage-patch-is-growing/40612/" target="_blank"> or here </a></p>')

        } else if (question3Answer === 'existingPrograms') {
            $('.global').toggleClass('hidden')
            $('.results').html('<h2>Help Doctors without Borders</h2> <p>Jeff could fund Doctors without borders foor over 2 years <a href="https://www.doctorswithoutborders.ca/msf-2018-facts-and-figures" target="_blank">more information here</a></p>')
            ...
            $('.indivigual').toggleClass('hidden')
            $('.results').html('<h2>Help the Homeless</h2> <p>With Jeff Bezozs\'s networth (on saturday september 12th 2020) he could end homelessness in the entire United States over 9 times <a href="https://www.huffingtonpost.ca/entry/homelessness-christmas-decorations_n_2276536?ri18n=true" target="_blank">more information here</a></p>')

        } else if (question3Answer === 'fairWages') {
            $('.indivigual').toggleClass('hidden')
            $('.results').html('<h2>Support for Workers!</h2> <p>If Jeff Bezos, decided to give every single employee of Amazon — some 876,000 people — a one-time pandemic bonus of $105,000 bonus. he would have roughly the same networth he had prier to Covid 19 <a href="https://the.ink/p/billionaire-wealth-just-got-wealthier" target="_blank">more information here</a></p>')
